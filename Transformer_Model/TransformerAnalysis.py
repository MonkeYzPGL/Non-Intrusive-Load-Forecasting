...
from Transformer_Model.Transformer import TransformerModel
...
class TransformerAnalyzer:
    timing_csv = "training_timing_transformer.csv"
    ...
    def custom_loss(self, y_pred, y_true, alpha=3, beta=5):
        base_loss = self.criterion(y_pred, y_true).mean(dim=1).mean()
    ...
    def train(self, epochs=100, patience=15, model_path=None):
        ...
        for epoch in range(epochs):
            ...
            if val_losses[-1] < best_val_loss:
                if model_path is not None:
                    ...
                    model_save_path = "saved_transformer_model.pth"
        ...
        print(f"\n Timp total pentru antrenare aparat {self.channel_number}: {training_duration:.2f} secunde")
