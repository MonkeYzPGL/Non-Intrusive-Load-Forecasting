...
def run_total_forecast_for_day(target_day, base_dir):
...
def run_forecast_for_single_channel(target_day, channel_id, base_dir):
...
def calculate_forecast_metrics(channel_id, date_str, base_dir):
    import os
    import pandas as pd
    from Services.ErrorMetricsService import ErrorMetricsAnalyzer

    if channel_id == 1:
        forecast_path = os.path.join(base_dir, "predictii_viitor", date_str, "combinat", "forecast_total.csv")
