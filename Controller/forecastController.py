...
import os
import pandas as pd
from Services.LSTMForecastService import run_total_forecast_for_day, run_forecast_for_single_channel, \
...
def get_prediction_for_day(channel_id, date_str):
    ...
    try:
        if channel_id == 1:
            if not os.path.exists(path_total):
                run_total_forecast_for_day(date_str, BASE_DIR)
            ...
            if not os.path.exists(path_single):
                df_forecast = run_forecast_for_single_channel(date_str, channel_id, BASE_DIR)
            else:
...
@forecast_bp.route("/predictie/metrics/<int:channel_id>/<date_str>", methods=["GET"])
def get_forecast_metrics(channel_id, date_str):
    try:
        BASE_DIR = r"C:\Users\elecf\Desktop\Licenta\Date\UK-DALE-disaggregated\house_1"
        metrics = calculate_forecast_metrics(channel_id, date_str, BASE_DIR)
