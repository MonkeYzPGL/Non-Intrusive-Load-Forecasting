...
class KAN(torch.nn.Module):
    def __init__(
        ...
        self.dropouts = torch.nnModuleList()
    ...
    def forward(self, x: torch.Tensor, update_grid=False):
        for layer, dropout in zip(self.layers, self.dropouts):
            if update_grid:
                layer.update_grid(x)
            x = layer(x)
            x = dropout(x)  # 🔹 Dropout aplicat după fiecare strat
        return x
...
class KANTimeSeriesModel(torch.nn.Module):
    def __init__(self, input_dim, hidden_dims=[128, 64], output_dim=1, dropout=0.2):
        super().__init__()
        self.model = KAN([input_dim] + hidden_dims + [output_dim],
                         grid_size=20, spline_order=3,
                         dropout=dropout)  # 🔹 trimitem dropout
